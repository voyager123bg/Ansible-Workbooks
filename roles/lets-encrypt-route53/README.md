...
Amazon's Route 53 (AWS). Auto-renew is also in.

Please note that as part of this role, `openssl`, [boto](https://github.com/boto/boto), and
[pyOpenSSL](https://github.com/pyca/pyopenssl) will be installed. Also ansible will be installed, naturally, as we already have a solution to issue cert, based on route53 challenge. How nice!

WARNING: Please note - this role may not be suitable to use on public Internet-facing servers, as the way it basically function is - it takes your AWS secrets out of the vault, creates static playbook on target host (that renews the certificate), and executes this static renew certificate playbook via cron. (Inspired by how worms propagate...)
...
Following ansible roles (avail. in galaxy) required:
- geerlingguy.ansible
- geerlingguy.pip
...
* **ler53_aws_access_key** - AWS access key (needed for route53)
* **ler53_aws_secret_key** - AWS access secret (needed for route53)
...
* **ler53_cron_renew** - schedule for cert renewal.
...
## Credits
 - Nikolay Georgiev
 - Original: Mat Prahl - https://github.com/mprahl/ansible-role-lets-encrypt-route-53/